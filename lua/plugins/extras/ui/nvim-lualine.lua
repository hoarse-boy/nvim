...
local lsp = {
  function()
    ...
    --   -- print("Attached " .. client.name .. " to buffer " .. bufnr)
    --   -- return "  " + client.name
    --   -- return string.format("  %s", client.name)
    --   language_server = string.format("  %s", client.name)
    -- end)

    -- print("kabom " + language_server)
...
--       return "LSP Inactive"
--     else
--       -- loop trough all clients and compare with current buffer client and return that instead
--       for _, client in pairs(clients) do
--         if client.name ~= "null-ls" and client.name ~= "copilot" then
--           if client.name == vim.bo.filetype then
--             return " " .. client.name -- Add symbol before the LSP name
--           end
--         end
--       end
--       -- return " " .. clients[1].name -- Add symbol before the LSP name
--     end
--   end,
--   color = { fg = "#62a0c4", gui = "bold" },
-- }
